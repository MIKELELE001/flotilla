...
async function main() {
  ...
  // One round-trip: positions, open orders, and recent trades together,
  // each position already carrying its market context (asset, status,
  // expiry, human interval label).
  ...
  // ---- Dashboard numbers: total at risk, net UP/DOWN exposure ----
  ...
  // ---- Claimable winnings: settled markets, aggregated, ready for redeemMany ----
  ...
  // Only positions in markets that have actually settled are candidates —
  // getPortfolio's `positions` list can include still-open ones too.
  const claimableInputs: ClaimableInput[] = [];
  for (const p of portfolio.positions) {
    if (!settledIds.has(p.market.id)) continue;
    if (!isHexId(p.market.id)) continue; // guard: SDK types this as a general string
  ...
  if (claimable.length > 0) {
    // Decimals vary per market; report raw here, format per-market in the real UI.
